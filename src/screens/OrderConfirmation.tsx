...
export function OrderConfirmation() {
        ...
        setOrder({
          ...
          items: orderData.items.map((item: any) => ({
            ...
            product: {
              ...
              image: item.product?.image || item.productId?.image || 'https://via.placeholder.com/80',
              cashback_percentage: item.product?.cashback_percentage || item.productId?.cashback_percentage || 0
            }
          }))
        })
      } catch (err) {
  ...
  const calculateTotalCashback = () => {
    return order.items.reduce((total, item) => {
      return total + (item.product.price * item.quantity * item.product.cashback_percentage) / 100
    }, 0)
  }
  ...
  return (
    <ScrollView flex={1} bg="gray.50" p={4}>
      <VStack space={4} bg="white" p={4} borderRadius="md" shadow={1}>
        ...
        <Text color="gray.500">Número do pedido: #{order.id.substring(0, 8).toUpperCase()}</Text>
        <Text color="gray.500">Data: {new Date(order.createdAt).toLocaleDateString('pt-BR')}</Text>
        ...
        {order.items.length > 0 ? (
          <VStack space={4}>
            {order.items.map((item) => (
              <HStack key={`${item.id}-${item.product.id}`} space={3} alignItems="center">
                <Image
        ...
        <VStack space={2}>
          ...
          <HStack justifyContent="space-between">
            ...
            <Text color="green.600">{formatCurrency(calculateTotalCashback())}</Text>
          </HStack>
        </VStack>
