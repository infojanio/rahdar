...
export function OrderHistory() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        setLoading(true);
        console.log('Filtrando por status:', selectedStatus); // Para debug
        
        const response = await api.get('/orders/history', {
          params: { 
            status: selectedStatus || undefined // Envia o status para o backend
          },
        });
        
        console.log('Resposta da API:', response.data); // Para debug

        const validatedOrders = (response.data.orders || []).map((order: any) => ({
          id: order.id || '',
          ...
          items: (order.items || []).map((item: any) => ({
            ...
            product: {
              ...
              name: item.product?.name || item.productId?.name || 'Produto desconhecido',
              price: item.product?.price || item.productId?.price || 0,
              image: item.product?.image || item.productId?.image || DEFAULT_PRODUCT_IMAGE,
              cashbackPercentage: item.product?.cashbackPercentage || item.productId?.cashbackPercentage || 0
            }
          })),
    ...
    fetchOrders();
  }, [selectedStatus]); // Executa quando selectedStatus muda
  ...
  return (
    <Box flex={1} bg="gray.50" p={4}>
      ...
      {orders.length === 0 ? (
        <Box bg="white" pb={280} borderRadius="md" alignItems={'center'} fontSize={'16'}>
          <Text color="red.500">
            {selectedStatus === '' 
              ? 'Nenhum pedido encontrado' 
              : `Nenhum pedido com status ${STATUS_OPTIONS.find(o => o.value === selectedStatus)?.label.toLowerCase()}`
            }
          </Text>
        </Box>
      ) : (
            ...
            <Box mb={4} bg="white" p={4} borderRadius="md" shadow={1}>
              ...
              <VStack space={3} mb={3}>
                {item.items.map((orderItem) => (
                  ...
                  >
                    <Image
                      source={{ uri: orderItem.product.image || DEFAULT_PRODUCT_IMAGE }}
                      alt={orderItem.product.name}
                      size="sm"
                      borderRadius="md"
                      fallbackElement={
                        <Box bg="gray.200" size="sm" borderRadius="md" justifyContent="center" alignItems="center">
              ...
              <VStack space={2}>
                ...
                <HStack justifyContent="space-between" mt={2}>
                  <Text fontWeight="bold">Total Final:</Text>
                  <Text fontWeight="bold">
                    {formatCurrency(item.totalAmount - calculateOrderCashback(item))}
                  </Text>
                </HStack>
              </VStack>
            </Box>
